...
- **Nextcloud 26 is required _before_ upgrading to PHP 8.2.**
  - Since Nextcloud 26 doesn't work with PHP 7.4, you should follow our previous [PHP 8.1 upgrade guide](./Upgrade-to-PHP-8.1-with-Apache) if you haven't already.
...
1. Disable all old PHP versions in Apache
    ...
    # If you were using PHP 8.0 before, also use
    a2disconf php8.0-fpm
    # If you were using PHP 8.1 before, also use
    a2disconf php8.1-fpm
    ```
2. Configure Apache to use PHP 8.2
    ```bash
    a2enconf php8.2-fpm
    ```
3. Restart Apache
